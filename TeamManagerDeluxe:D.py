 # '__init__' makes the player have a name, age, goals, and jersey nu$
class Player(object):
...
Do you want to start with a new team or open an existing team file? 
 
Enter the number of your choice to choose. 
...
while user_choice != 0:
        print"""What do you want to do? Enter the number of your choi$
        (1) Add a player?
        (2) Print all players?
        (3) Print average number of goals for all players?
        (4) Save player to a file?
        (0) Leave the program?
        """
        user_choice = int(raw_input())
        ...
        if user_choice == 1:
                ...
                position = raw_input()
                my_players.append(Player(player_name, player_age, pla$
        ...
        # fourth choice
        if user_choice == 4:
                print("Ok, what would you like to name the file? We'l$
                filename = raw_input()
